...
parser.add_argument('--test-dir', action='store', dest='test_dir',
                    help='testing file directory location', required=True)
...
list_dir = [x for x in os.walk(args.test_dir)]

#
# Write train.tsv file with query_text \t doc_relevant_text \t doc_irrelevant_text for all subdirectories
#
entailment_gtruth_dict = json.load(open(os.path.join(args.test_dir,'task2_test_labels_2021.json') ,"rb"))
with open(os.path.join(args.output_dir, 'test.tsv'), 'wt') as out_file:
    ...
    for sub_dir in list_dir[0][1]:
        # read in query text
        with open(os.path.join(args.test_dir, sub_dir, 'entailed_fragment.txt'), 'r') as entailed_fragment:
...
#         with open(os.path.join(args.test_dir, sub_dir, 'entailing_paragraphs.txt'), 'r') as entailing_paragraphs:
        ...
        list_sub_dir_paragraphs = [x for x in os.walk(os.path.join(args.test_dir, sub_dir, 'paragraphs'))]
        paragraphs_text = {}
        
        for paragraph in list_sub_dir_paragraphs[0][2]:
            with open(os.path.join(args.test_dir, sub_dir, 'paragraphs', paragraph), 'r') as paragraph_file:
